...
%\begin{rem}[Abelian categories]
%	Note that for an \textbf{abelian} category $\mathsf{C}$ we have $\mathrm{Hom}_{\mathsf{C}} \left(X, Y \right) \in \mathsf{Mod}\left( \Z \right)$, for ${X,Y \in \mathsf{C}}$.
%	This means that we can also have representable functors that take values in the category of modules, i.e.
%	$F: \mathsf{C}^{op} \to \mathsf{Mod}\left( \Z \right)$.
%\end{rem}
